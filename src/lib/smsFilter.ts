 ...
 *
 * الگوها عمداً کلمات را چسبیده نمی‌خواهند. یک نمونه واقعی که نسخه قبلی
 * را دور زد: «ترب، کد اختصاصی برای ورود ... code: 8131» — چون «کد» و
 * «ورود» با دو کلمه فاصله آمده بودند. حالا تا ۲۵ کاراکتر فاصله پذیرفته
 * می‌شود و شکل انگلیسی هم پوشش دارد.
 */
const OTP_PATTERNS = [
  // «code: 8131» یا «code 8131»
  /\bcode\s*[:：=]?\s*\d{3,8}\b/i,
  // «کد اختصاصی برای ورود»، «کد ورود شما»، «رمز یکبار مصرف»
  ...
  // «جهت ورود کد ۱۲۳۴ را وارد کنید»
  /(?:ورود|تایید|تأیید|احراز|عضویت|ثبت\s*نام)[^\n]{0,25}?(?:کد|رمز)\s*[:\s]*\d{3,8}/,
  // «کد اختصاصی» تقریباً همیشه کد ورود است، نه تخفیف
  /کد\s*اختصاصی/,
  /رمز\s*(?:عبور|ورود|موقت|یکبار|یک\s*بار|پویا|دوم)/,
...
const AUTH_INTENT =
  /ورود|تایید|تأیید|احراز|فعال\s*ساز|فعالساز|عضویت|ثبت\s*نام|اعتبارسنجی|شناسایی|رمز|\bcode\b|\blogin\b/i;

/** عدد تنهای ۴ تا ۸ رقمی — شکل معمول رمز یک‌بارمصرف */
const STANDALONE_CODE = /(?:^|[^\d])\d{4,8}(?:[^\d]|$)/;
...
const STRONG_SIGNALS = SIGNALS.filter((s) => s.weight >= 3).map((s) => s.pattern);
...
export function classifySms(
  ...
  /*
   * دام دوم برای رمزهای یک‌بارمصرفی که جمله‌بندی غیرمعمول دارند.
   *
   * پیامک کوتاه + یک عدد تنهای چندرقمی + کلمه‌ای با نیت ورود، و بدون هیچ
   * نشانه قوی تخفیف: عملاً همیشه کد ورود است. شرط «نشانه قوی نداشته باشد»
   * مهم است، چون پیامک تخفیف واقعی تقریباً همیشه کلمه‌ای مثل «تخفیف» یا
   * «کد تخفیف» دارد و نباید قربانی این قاعده شود.
   */
  const strongPromo = STRONG_SIGNALS.some((re) => re.test(text));
  if (
    !strongPromo &&
    text.length < 160 &&
    STANDALONE_CODE.test(text) &&
    AUTH_INTENT.test(text)
  ) {
    return {
      type: 'banking',
      ...empty,
      safeToSend: false,
      reason: 'به نظر کد ورود می‌رسد — روی گوشی باقی می‌ماند',
    };
  }
